...
c     write(*,*)'Argument:',string(1:40)
         ...
         if(string(1:1) .eq. '-') then
            if(string(1:2) .eq. '-e') collsum=.true.
...
c     Get current directory full filename
         dirjf=filename(jf)
         direndjf=0
         do j=1,nsmax
            if(dirjf(j:j).eq.'/') direndjf=j
         enddo
         dirjf=dirjf(1:direndjf)
...
c     Get the full name of the current file -> filei
            do j=1,nsmax-4
               if(charin(j:j+2).eq.'dat') then
                  read(charin(5:j+2),'(a)')
     $                 filei(direndjf+1:direndjf+j-2)
                  filei(1:direndjf)=dirjf(1:direndjf)
                  filei=filei(1:direndjf+j-2)
                  goto 13
               endif
            enddo
...
c    (Leo) Replace debye by debyelen, otherwise when we use a prespecified
c     potential debye=1e5 (because this is what is on the filename).
                     debye=debyelen
...
c     (Leo)Calculate the total momentum lost per unit time 
c     in the whole domain. (IHH) Compare vzsum and vr/tsum approaches.
               if(collsum) then

                  call readfile(filei,nrhere,nthhere)
                  pz=0.
                  pzvz=0.
                  vmoy=0.
                  do kk=1,nrhere
                     do jj=1,nthhere
                        pzvz=pzvz+vzsum(kk,jj)-psum(kk,jj)*vd
                        pz=pz+(vrsum(kk,jj)*tcc(jj)
     $                       -vtsum(kk,jj)*sqrt(1-tcc(jj)**2)
     $                       -psum(kk,jj)*vd)
                        vmoy=vmoy+vrsum(kk,jj)*tcc(jj)
     $                       -vtsum(kk,jj)*sqrt(1-tcc(jj)**2)
                     enddo
                  enddo
                  pz=pz/rhoinf
...
c We have a number of different collisionalities. Force:
               yleg=.95
               if(collsum)then
                  call pltinit(0.,1.,0.,1.)
                  call scalewn(1.e-4,10.,-30.,80.,.true.,.false.)
                  call axis()
                  call polymark(colnwtarr,ftot1,i,1)
                  call color(4)
                  call dashset(4)
                  call polymark(colnwtarr,ftot2,i,1)
                  call legendline(.0,yleg-.20,6,' Collisions')
                  call polymark(colnwtarr,ftot2,i,1)
...
c Floating potential plot
     ...
     $    'Sphere Potential /[T!de!d/e]')
            ...
            call minmax(ftot2,i,fmin2,fmax2)
...
c            call ticset(.015,.015,-.03,-.03,0,0,0,0)
c            call yaxis(1.,0.)
c            call xaxis(0.,1.)
c            call ticset(.01,.01,-.03,-.03,0,0,0,0)
c            call ticlabtog()
c            call xaxis(0.,.5)
c            call ticlabtog()
c            call axis2()
...
c     call dashset(5)
 ...
 99   write(*,*)'Usage: ./forcesum [switches] file1, file2, ...\n',
     ...
     $     '-vf.f set tempmul (1). -n no screen output. -e collforce'
...
c__________________________________________________________________
      ...
      open(11,file=filename,status='old',err=210)
c     Line for nothing.
      read(11,*)charin
      read(11,'(a)')charin
c     write(*,*)charin
...
c      write(*,'(a,a)')'  dt    vd     Ti     steps  rhoinf ' ,
c     $     'phiinf  fave  debyelen Vp damplen  Bz...'
c      write(*,'(2f7.4,f7.3,i5,f8.1,f7.3,f8.4,f8.3,f8.3,f6.2,f7.3,$)')
c     $     dt,vd,Ti,isteps,rhoinf,phiinf,fave,debyelen,vprobe,damplen,Bz
      if(icolntype.gt.0)then
c         write(*,'(i2,f7.3)',err=212)icolntype,colnwt
      else
c         write(*,*)
      endif

 212  read(11,*,err=202)nrTi
      nrhere=nrTi
c      write(*,*)'nrTi=',nrTi
      do i=1,nrTi
         read(11,*,err=203)rpic(i),phipic(i)
      enddo
      read(11,*)charin
c      write(*,*)charin(1:78)
      read(11,*,err=204)nsteps
c      write(*,*)nsteps
      if(nsteps.gt.nstepmax) then
c         write(*,*)'Number of steps',nsteps,
c     $        ' exceeds allocation',nstepmax
         call exit
      endif
      read(11,*)(fluxprobe(j),j=1,nsteps)
c Read theta cells
      read(11,*)charin
c      write(*,*)charin(1:78)
      read(11,*)nthhere,nsteps
c      write(*,*)nthhere,nsteps
      do i=1,nsteps
         read(11,*)(ninthstep(j,i),j=1,nthhere)
      enddo
...
c      if(.not.lpcic)nread=nread+1
      read(11,*)(ninth(j),j=1,nread)
      if(lreaddiag)then
c         write(*,*)'nastep',nastep,' ninth:'
c         write(*,*)( ninth(j),j=1,nthhere)
      endif

      read(11,*)charin
      do j=1,nrhere
         read(11,*)(phi(j,k),k=1,nthhere)
      enddo
      read(11,*)charin
      do j=1,nrhere
         read(11,'(10f8.3)')(rholocal(j,k),k=1,nthhere)
      enddo
      read(11,*)charin
      read(11,*)(volinv(k),k=1,nrhere)
...
c Read in  summed results.
         filename(1:2)='Sp'
         open(11,file=filename,err=210,status='old')
      endif
      read(11,'(a)')string
      read(11,*,err=200)
     $     dt,vd,Ti,i,rmax,rhoinf,debyelen,vprobe
 200  continue
      read(11,*)nrhere,nthhere,nphere

c      if(nrhere.gt.NRUSED .or. nthhere.gt.NTHUSED)then
c         write(*,*)'Required dimensions: nr',nrhere,' nth',nthhere
c         write(*,*)'are too large for the allocated values:'
c     $        ,NRUSED,NTHUSED
c         stop
c      endif
      
      read(11,*)string
      read(11,*)((psum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((vrsum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((vtsum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((vpsum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((v2sum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((vr2sum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)((vtp2sum(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      if(string(1:5).eq.'vzsum')then
         read(11,*)((vzsum(k1,k2),k1=1,nrhere),k2=1,nthhere)
         read(11,*)string
      endif
      read(11,*)((diagvr(k1,k2),k1=1,nrhere),k2=1,nthhere)
      read(11,*)string
      read(11,*)(rcc(k1),k1=1,nrhere)
      read(11,*)string
      read(11,*)(volinv(k1),k1=1,nrhere)
      read(11,*)string
      read(11,*)(tcc(k2),k2=1,nthhere)      
      read(11,*,err=402,end=402)string
      read(11,*)charge1,ffield1,felec1,fion1,ftot1
      read(11,*)charge2,ffield2,felec2,fion2,ftot2
      read(11,*,err=402,end=402)string
      read(11,*,err=402,end=402)
